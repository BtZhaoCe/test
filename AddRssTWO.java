...
public class AddRss extends Activity {
	...
	OnClickListener clickListener = new OnClickListener() {		
		...
		public void onClick(View v) {
			...
			case R.id.verify_rss:
				if((rssText.getText().toString()) != null )
				{new verifyRss().execute((rssText.getText().toString()));
	...
	protected void saveRss() {
		...
		String rssName = null;
		try {
			rssName = new verifyRss().execute(rssAddress).get();
		...
		//RSS源地址正确并正确保存
		if(rssName != null && (-1 != mChannelData.SaveChannelInfo(rssName, rssAddress)))
		{
	...
	private class verifyRss extends AsyncTask<String, Void, String>
	{
